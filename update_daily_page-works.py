...
def get_csv_statistics(csv_path):
    ...
    Returns:
        dict: 통계 정보 딕셔너리 또는 None (파일 없음)
              keys: first_date, last_date, count, last_close, last_marcap
    """
    ...
    try:
        with open(csv_path, 'r', encoding='utf-8-sig') as f:
            ...
            # 시가총액 (최신 데이터에 없을 수 있음 → 뒤에서부터 탐색)
            stats['last_marcap'] = None
            for row in reversed(rows):
                marcap = row.get('marcap', '')
                if marcap and marcap.strip():
                    try:
                        stats['last_marcap'] = float(marcap)
                        break
                    except ValueError:
                        continue
...
def generate_data_item_html(ticker, company, stats, csv_filename):
    ...
    marcap_str = format_marcap(stats.get('last_marcap'))
